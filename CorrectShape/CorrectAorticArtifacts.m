...
[xFinal,yFinal] = CheckAortaCurvature(Img,x,y,c1,c2);
...
function [xFinal,yFinal] =CheckAortaCurvature(Img,x,y,c1,c2);
...
Vectorn = [xNew(4:length(x)+3)-xNew(1:length(x)); yNew(4:length(y)+3)-yNew(1:length(y)); zeros(1,length(x))];
VectorR = [xNew(1:length(x)); yNew(1:length(y)); zeros(1,length(x))];
theta_Vectorn_VectorR=acos( (VectorR(1,:).*Vectorn(1,:)+VectorR(2,:).*Vectorn(2,:))./sqrt(( VectorR(1,:).^2+VectorR(2,:).^2).*( Vectorn(1,:).^2+Vectorn(2,:).^2)));

IndexOfCheckPoint = find ((theta_Vectorn_VectorR*180/pi)<45  |  (theta_Vectorn_VectorR*180/pi)>150) ;
...
% plot(ChangeSequenceX,ChangeSequenceY, [ChangeSequenceX(1),ChangeSequenceX(i)], [ChangeSequenceY(1),ChangeSequenceY(i)],'r',[ChangeSequenceX(1),ChangeSequenceX(4)],[ChangeSequenceY(1),ChangeSequenceY(4)],'m')
 %  pause(1)
   ...
   if (Rxn(1,3)*Kxn(1,3)>0) || (Theta_Kn <70 && (Rxn(1,3)*Kxn(1,3))<=0 )|| Theta_RK<35 || (Theta_Rn_<50 || Theta_Rn_>145)
        n=n+1;
        KeepIndeces(n)=i;
        k=1;
    end
